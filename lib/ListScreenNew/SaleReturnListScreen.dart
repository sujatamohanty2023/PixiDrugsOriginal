...
class SaleReturnScreen extends StatefulWidget {
  ...
  const SaleReturnScreen({Key? key}) : super(key: key);

  @override
  State<SaleReturnScreen> createState() => _SaleReturnScreenState();
}

class _SaleReturnScreenState extends State<SaleReturnScreen> with WidgetsBindingObserver, RouteAware {
  String searchQuery = "";
  ...
  Future<void> _fetchRecord({bool refresh = false}) async {
    ...
    await context.read<ApiCubit>().fetchSaleReturnList(store_id: userId, page: currentPage,from:from??'',to:to??'',reason:selectedPaymentReason,filter: searchQuery);
    ...
    return Scaffold(
      body: BlocBuilder<ApiCubit, ApiState>(
        builder: (context, state) {
          final isLoading = state is SaleReturnListLoading;

          // Populate lists from state
          if (state is SaleReturnListLoaded) {
                ...
                Expanded(
                  child: RefreshIndicator(
                    ...
                    child: Container(
                      ...
                      child:(isLoading || isRefresh) && saleReturnList.isEmpty?
                      Center(child: CircularProgressIndicator(color: AppColors.kPrimary))
                          : (!isLoading && !isRefresh) && saleReturnList.isEmpty
                          ? NoItemPage(
                        onTap: (){},
  ...
  Widget _buildListBody(bool isLoading) {
    final screenWidth = MediaQuery.of(context).size.width;
    final itemCount = saleReturnList.length + (hasMoreData ? 1 : 0);
    return MediaQuery.removePadding(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return _buildReturnCard(saleReturnList[index], screenWidth,context);
        },
      ),
    );
  }
